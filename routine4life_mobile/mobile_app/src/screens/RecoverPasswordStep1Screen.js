...
const RecoverPasswordStep1Screen = ({ navigation }) => {
  ...
  const handleSendMessage = () => {
    if (isButtonEnabled) {
      ...
      // Pasamos el email (si existe) al siguiente paso
      navigation.navigate('RecoverPasswordStep2', { email: email.trim() });
    }
  };
...
// ... (Los estilos se mantienen exactamente igual que los tuyos, están perfectos)
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: COLORS.background },
  scrollContainer: { flexGrow: 1, paddingHorizontal: 30, paddingTop: '20%', alignItems: 'center' },
  headerContainer: { marginBottom: 60 },
  title: { fontSize: 50, fontWeight: 'bold', color: COLORS.primary, textAlign: 'center', lineHeight: 55 },
  formContainer: { width: '100%', marginBottom: 60 },
